...
class AddPetInfoController extends GetxController {
  ...
  RxList<BreedModel> breedFilterList = RxList();
  ...
  /*onBirthDateChanges() {
    try {
      ...
      debugPrint('BDAY: $bDay');
      petBirthDate(bDay.formatDateYYYYmmdd());
      debugPrint('PET BIRTHDATE: $petBirthDate');
    } catch (e) {
  ...
  void _getFromGallery() async {
    ...
    if (pickedFile != null) {
      ...
      debugPrint('ADDPETREQ.PETIMAGES: ${addPetReq.petImages.validate().length}');
  ...
  addPetApi() async {
    ...
    debugPrint('ADDPETREQ.TOJSON(): ${addPetReq.toJson()}');
  ...
  getPetTypesApi() async {
    isLoading(true);
    hideKeyBoardWithoutContext();
    PetService.getPetTypeApi().then((value) async {
      hasErrorFetchingbookingList(false);
      petTypeList(value.data);
      if (isUpdateProfile.value) {
      ...
      setValueToLocal(APICacheConst.PET_TYPES, value.toJson());
      isLoading(false);
    }).catchError((e) {
      isLoading(false);
  ...
  void searchFunc({
    ...
    required RxList<BreedModel> breedFilterList,
    required RxList<BreedModel> breedSList,
  }) {
    breedFilterList.value = List.from(breedSList.where((element) => element.name.toString().toLowerCase().contains(searchtext.toString().toLowerCase())));
    for (var i = 0; i < breedFilterList.length; i++) {
      debugPrint('SEARCHEDNAMES : ${breedFilterList[i].toJson()}');
    }
    debugPrint('SEARCHEDNAMES.LENGTH: ${breedFilterList.length}');
  }

  void onSearchChange(searchtext) {
    searchFunc(
      searchtext: searchtext,
      breedFilterList: breedFilterList,
      breedSList: breedList,
    );
  }
  ...
  getBreed() {
    ...
    isLoading(true);
    PetServiceFormApis.getBreed(petTypeId: selectedPetType.value.id).then((value) {
      isLoading(false);
  ...
  bool get isShowFullList => breedFilterList.isEmpty && searchCont.text.trim().isEmpty;
}
