...
class WishListApis {
  static Future<bool> addToWishList({required int productId}) async {
    try {
      final res = BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse(APIEndPoints.addToWishList, method: HttpMethodType.POST, request: {ProductModelKey.productId: productId})));
      return res.status;
    } catch (e) {
      toast(e.toString());
      return false;
  ...
  static Future<bool> removeFromWishList({required int productId}) async {
    try {
      final res = BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse('${APIEndPoints.removeWishList}?product_id=$productId', method: HttpMethodType.GET)));
      return res.status;
    } catch (e) {
      toast(e.toString());
      return false;
  ...
  static Future<bool> onTapFavourite({required ProductItemData favdata}) async {
    ...
    if (favdata.inWishlist.value) {
      ...
      if (success) {
        isRemoved = true;
        toast(locale.value.productRemoveToWishlist);
      ...
      if (success) {
        ...
        toast(locale.value.productAddedToWishlist);
        updateProductElementEveryWhere();
