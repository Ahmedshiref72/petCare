...
import 'package:pawlly/screens/home/model/event_model.dart';
...
class HomeServiceApis {
  ...
  static Future<StatusListRes> getAllStatusUsedForBooking({List<String>? statusList}) async {
    return StatusListRes.fromJson(await handleResponse(await buildHttpResponse("${APIEndPoints.bookingStatus}?${statusList.validate().join(',')}", method: HttpMethodType.GET)));
  }
  ...
  static Future<List<PetEvent>> getEvent({
    ...
    if (isLoggedIn.value) {
      final eventRes = EventRes.fromJson(await handleResponse(await buildHttpResponse("${APIEndPoints.getEvents}?per_page=$perPage&page=$page", method: HttpMethodType.GET)));
